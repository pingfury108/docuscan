...
class DocumentScanner:
    ...
    def scan_document(
        ...
        try:
            ...
            if processing_config["output"]["white_background"]:
                logger.info("创建白底文档...")
                current_image = (
                    self.background_processor.create_white_background_document(
                        current_image
                    )
                )
                if return_intermediate:
            ...
            if processing_config["binarization"]["enable"]:
                ...
                current_image = self.binarizer.binarize(
                    ...
                    **{
                        k: v
                        for k, v in processing_config["binarization"].items()
                        if k != "enable"
                    },
                )
                if return_intermediate:
            ...
            # 最终裁剪白色边框
            final_image = self.geometric_corrector.auto_crop_white_borders(
                current_image
            )
            ...
            logger.info(
                f"文档扫描处理完成，从 {original_size} 处理到 {final_image.shape[:2]}"
            )
            return result
    ...
    def scan_for_ocr(self, image: np.ndarray) -> np.ndarray:
        ...
        ocr_config = {
            ...
            "output": {"white_background": True, "max_width": 2000, "max_height": 2000},
        }
    ...
    def detect_document_quality(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_report["illumination"] = {
                ...
                "suggestion": "光照不均匀，建议调整光源"
                if illumination_var >= 3000
                else "光照均匀度良好",
            }
